...
static ssize_t unikernel_mem_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
  	...
  	if ( ret != 0 ) return 0 ;
		...
		ret += sprintf(buf+ret, "%3d\n", (az_info.unikernels[id].meminfo)->to ) ;
...
static ssize_t unikernel_info_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
		...
		ret += sprintf(buf+ret, "%s\n", az_info.unikernels[id].filename ) ;
...
static struct attribute *azalea_attrs[] = {
			&azalea_meminfo.attr,
			&azalea_cpuinfo.attr,
			&azalea_available_cpu.attr,
			&azalea_available_mem.attr,
			NULL,
} ;
...
void sysfs_azalea_exit(void)
{
  int i;
    ...
    if ( az_info.unikernels[i].avail == FALSE )
      {
				if ( az_info.unikernels[i].unikernel_kobj != NULL )
						kobject_put(az_info.unikernels[i].unikernel_kobj) ;
			}
